...
// @Summary Api to update kyc consent
// @Tags KYC Apis
// @Accept  json
// @Produce  json
// @Security ApiKeyAuth
...
// @Param requestBody body requests.KycConsentRequest true "Request Body Data"
...
// @Router /api/kyc/consent [post]
func UpdateKycConsent(c *gin.Context) {
	...
	request := requests.NewKycConsentRequest()
	...
	if err := store.Kyc.UpdateKycConsent(c.Request.Context(), authValues, request); err != nil {
		responses.StatusBadRequest(
			c,
			customerror.NewError(err),
			"",
	...
	responses.StatusOk(
		c,
		nil,
		"successfully updated kyc consent",
		"",
	)
}
